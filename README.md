...
Вот детальное описание mock-эндпоинтов для вашего API:

### **Mock API Endpoints**

Эти эндпоинты имитируют работу с постами (статьями) в системе, демонстрируя как может применяться система аутентификации/авторизации к бизнес-объектам.
...
**Описание:**  
Возвращает список всех mock-постов в системе.

**Требования:**

- Действительный access token (пользователь должен быть авторизован)
...
**Эндпоинт:**  
`GET /mock/posts/{post_id}`

**Описание:**  
Возвращает конкретный пост по ID с проверкой прав доступа:

- Автор поста может просматривать свой пост
- Администратор может просматривать любые посты
- Другие пользователи получат ошибку 403

**Параметры:**

- `post_id` (в URL) - ID запрашиваемого поста
...
- `401 Unauthorized` - если пользователь не авторизован
- `403 Forbidden` - если пользователь не автор и не админ
...
Эти эндпоинты демонстрируют:

1. **Аутентификацию** - все запросы требуют валидного токена
2. **Авторизацию** - проверка `author_id` vs `user.id` + роль `admin`
